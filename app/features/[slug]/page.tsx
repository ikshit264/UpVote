import { getFeatureBySlug, getAllFeatures, type FeatureImage, type FeatureSection } from "@/lib/features";
...
function FeatureFigure({ image }: { image: FeatureImage }) {
    return (
...
function SectionImages({ images, position }: { images?: FeatureImage[]; position: "before" | "after" }) {
    ...
    return (
        <div className={filtered.length > 1 ? "my-10 grid grid-cols-1 gap-6 sm:grid-cols-2" : ""}>
            {filtered.map((img, idx) => (
                <FeatureFigure key={`${position}-${idx}-${img.src}`} image={img} />
            ))}
        </div>
    );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    if (!feature) {
        return { title: "Feature Not Found | MonkFeed" };
    }
    ...
    return {
        ...
        keywords: feature.metadata.keywords,
...
export default async function FeatureDetailPage({ params }: Props) {
    ...
    return (
        <div className="min-h-screen bg-white text-zinc-900 selection:bg-blue-500/20">
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        itemListElement: [
                            ...
                            { "@type": "ListItem", position: 3, name: feature.title, item: `${baseUrl}/features/${feature.slug}` },
                        ],
                    }),
                }}
            />
                    ...
                    <div className="mx-auto max-w-4xl space-y-8">
                        ...
                        <div className="relative aspect-[9/4] overflow-hidden rounded-[2rem] border border-zinc-200 bg-zinc-50 shadow-xl">
                            <Image
                                ...
                                title={feature.imageAlt || feature.title}
                                fill
            ...
            ">
                            {feature.sections.map((section: FeatureSection, idx: number) => {
                        ...
                        <div className="relative mt-20 overflow-hidden rounded-[2.5rem] bg-zinc-900 p-10 shadow-xl">
                            ...
                            <div className="relative z-10 space-y-6">
                                <h3 className="text-3xl font-bold text-white">Ready to put {feature.title} to work?</h3>
