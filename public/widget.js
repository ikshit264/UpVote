(function () {
  ...
  if (!config.applicationId || !config.userId || !config.email) {
  ...
  const feedbackSub = createSubButton('Feedback', '💬', '#818cf8', '#6366f1', 'upvote-feedback-btn');
  const supportSub = createSubButton('Support', '🎧', '#34d399', '#10b981', 'upvote-support-btn');
  ...
  function showMenu() {
    menuOpen = true;
    // Feedback slides up first with wave curve
    ...
    // Support slides up second with slight delay for stagger
    setTimeout(() => {
    ...
    // Labels fade in after buttons land
    setTimeout(() => {
    ...
    // Rotate main button to indicate open
  ...
  // Close when receiving message from iframe
  ...
  // Close menu or widget when clicking outside
